...
mintlib: libc-${TARGET}.ok
	...
	cd ${FOLDER_MINTLIB} && \
		export PATH=${INSTALL_DIR}/bin:$$PATH && \
		$(MAKE) OUT= toolprefix=${TARGET}- SHELL=$(BASH) CROSS=yes WITH_020_LIB=yes WITH_V4E_LIB=yes $(OUT)
	cd ${FOLDER_MINTLIB} && \
		export PATH=${INSTALL_DIR}/bin:$$PATH && \
		$(MAKE) OUT= toolprefix=${TARGET}- SHELL=$(BASH) CROSS=yes WITH_020_LIB=yes WITH_V4E_LIB=yes install $(OUT)
